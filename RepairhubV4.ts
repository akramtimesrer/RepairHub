...
import { 
  ...
  // Existing Imports from Source B
  Phone, UploadCloud, XOctagon, Layout, Globe, ShieldOff, MessageSquare,
  // Existing Imports for Premium Features
  Crown, History,
  // NEW IMPORTS FROM PATCH
...
export default function RepairMarketplace() {
  ...
  const [modals, setModals] = useState({
    ...
    assistant: false, pricing: false, settings: false // Added settings
  });
...
function AIContextAssistant({ user, machines, onClose, darkMode }) {
    ...
    return (
        <div className="fixed bottom-24 right-6 w-96 h-[600px] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl flex flex-col border border-gray-200 dark:border-gray-700 overflow-hidden z-40 animate-fade-in-up">
            {/* Header */}
            ...
            <div className="p-3 border-t dark:border-gray-700 bg-white dark:bg-gray-800">
                {attachedFile && (
                    <div className="flex items-center justify-between text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded-lg mb-2">
    ...
    return (
        <div className="animate-fade-in max-w-6xl mx-auto">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold flex items-center gap-2"><Activity className="w-8 h-8 text-blue-600"/> IoT Monitor <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full uppercase tracking-wider">Premium</span></h1>
                    <p className="text-gray-500">Real-time Digital Twin & AI Diagnostics</p>
                </div>
                <div className="flex items-center gap-3 bg-white dark:bg-gray-800 p-2 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                    <span className="text-xs font-bold uppercase text-gray-500 ml-2">Simulate:</span>
                    <button onClick={() => setMode('Normal')} className={`px-3 py-1 rounded-lg text-xs font-bold transition ${mode === 'Normal' ? 'bg-green-100 text-green-700' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Normal</button>
                    <button onClick={() => setMode('Warning')} className={`px-3 py-1 rounded-lg text-xs font-bold transition ${mode === 'Warning' ? 'bg-yellow-100 text-yellow-700' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Drift</button>
                    <button onClick={() => setMode('Critical')} className={`px-3 py-1 rounded-lg text-xs font-bold transition ${mode === 'Critical' ? 'bg-red-100 text-red-700' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Failure</button>
                </div>
            </div>
            ...
            <div className="grid lg:grid-cols-4 gap-6">
                <div className="lg:col-span-1 space-y-3">
                    {machines.map(m => (
                        <div key={m.id} onClick={() => { setSelectedId(m.id); setHistory([]); setAiAnalysis(null); }} className={`p-4 rounded-xl border cursor-pointer transition flex justify-between items-center ${selectedId === m.id ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-blue-300'}`}>
                            <div>
                                <div className="font-bold text-sm">{m.name}</div>
                                <div className={`text-xs ${selectedId === m.id ? 'text-blue-200' : 'text-gray-500'}`}>{m.type}</div>
                            </div>
                            <div className={`w-2 h-2 rounded-full ${m.temp > 90 ? 'bg-red-500 animate-ping' : 'bg-green-400'}`}></div>
                        </div>
                    ))}
                    <button className="w-full py-3 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl text-gray-400 hover:text-blue-500 hover:border-blue-400 font-bold text-sm flex items-center justify-center gap-2 transition"><Plus className="w-4 h-4"/> Connect Sensor</button>
                </div>
                ...
                <div className="lg:col-span-3 space-y-6">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="font-bold text-lg flex items-center gap-2"><Activity className="w-5 h-5 text-green-500"/> Live Telemetry: {activeMachine?.name}</h2>
                            <div className="flex items-center gap-4 text-sm">
                                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-red-500 rounded-full"></div> Temp (°C)</div>
                                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-blue-500 rounded-full"></div> Vibration (Hz)</div>
                            </div>
                        </div>
                        
                        <div className="h-64 w-full bg-gray-50 dark:bg-gray-900 rounded-xl relative flex items-end px-2 overflow-hidden">
                            <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                                <polyline 
                                    fill="none" 
                                    stroke="#ef4444" 
                                    strokeWidth="2" 
                                    points={history.map((h, i) => `${(i / 19) * 100},${100 - Math.min(h.temp, 100)}`).join(' ')} 
                                />
                                <polyline 
                                    fill="none" 
                                    stroke="#3b82f6" 
                                    strokeWidth="2" 
                                    points={history.map((h, i) => `${(i / 19) * 100},${100 - (Math.min(h.vibe * 10, 100))}`).join(' ')} 
                                />
                            </svg>
                        ...
                        <div className="grid grid-cols-3 gap-4 mt-6">
                            <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-xl text-center">
                                <div className="text-gray-500 text-xs uppercase font-bold mb-1">Temperature</div>
                                <div className={`text-2xl font-mono font-bold ${activeMachine?.temp > 80 ? 'text-red-600' : 'text-gray-800 dark:text-gray-200'}`}>{activeMachine?.temp.toFixed(1)}°C</div>
                            </div>
                            <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-xl text-center">
                                <div className="text-gray-500 text-xs uppercase font-bold mb-1">Vibration</div>
                                <div className={`text-2xl font-mono font-bold ${activeMachine?.vibe > 5 ? 'text-red-600' : 'text-gray-800 dark:text-gray-200'}`}>{activeMachine?.vibe.toFixed(2)}Hz</div>
                            </div>
                             <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-xl text-center">
                                <div className="text-gray-500 text-xs uppercase font-bold mb-1">Uptime</div>
                    ...
                    <div className="bg-gradient-to-br from-indigo-900 to-blue-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
                        <div className="relative z-10 flex justify-between items-start">
                            <div>
                                <h3 className="font-bold text-xl flex items-center gap-2"><BrainCircuit className="w-6 h-6"/> AI Watchdog</h3>
                                <p className="text-blue-200 text-sm mt-1 max-w-md">Continuous monitoring for anomaly detection. Click analyze to run a deep diagnostic scan on the current data stream.</p>
                            </div>
                            <button onClick={runAIWatchdog} disabled={analyzing} className="bg-white text-blue-900 px-6 py-2 rounded-lg font-bold hover:bg-blue-50 transition shadow-lg disabled:opacity-50">
                                {analyzing ? 'Scanning...' : 'Analyze Stream'}
                            </button>
                        </div>
                        
                        {aiAnalysis && (
                            <div className="mt-6 bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 animate-fade-in">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-3 h-3 rounded-full ${aiAnalysis.status === 'Normal' ? 'bg-green-400' : aiAnalysis.status === 'Warning' ? 'bg-yellow-400' : 'bg-red-500'}`}></div>
                                        <span className="font-bold text-lg">{aiAnalysis.status}</span>
                                    </div>
                                    <div className="font-bold text-2xl">{aiAnalysis.health_score}<span className="text-sm text-blue-200 font-normal">/100</span></div>
                                </div>
                                <p className="text-blue-100 text-sm leading-relaxed">{aiAnalysis.advice}</p>
                                {aiAnalysis.urgent_action && <div className="mt-3 flex items-center gap-2 text-yellow-300 font-bold text-xs uppercase tracking-wide animate-pulse"><AlertTriangle className="w-4 h-4"/> Auto-Dispatching Technician...</div>}
                            </div>
                        )}
                        
                        <div className="absolute -bottom-10 -right-10 text-white opacity-5"><BrainCircuit className="w-64 h-64"/></div>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                        ...
                        <div className="space-y-4">
                            ...
                            <button 
                                ...
                                disabled={analyzing}
                                className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2 ${analyzing ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:scale-[1.02]'}`}
                            >
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    {result && (
                        <div className="animate-slide-in space-y-6">
                            ...
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                                <h3 className="font-bold text-xl mb-4 flex items-center gap-2 text-green-600"><CheckCircle className="w-5 h-5"/> Repair Guide: {result.suspected_component}</h3>
                                <div className="space-y-4">
...
function ContractViewer({ contract, currentUser, onSign, onUpdate, onClose, onReleaseFunds, darkMode }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl w-full max-w-3xl h-[90vh] flex flex-col shadow-2xl relative">
                 <div className="text-center mb-6 border-b dark:border-gray-700 pb-4">
                      <h2 className="text-2xl font-serif font-bold text-gray-900 dark:text-white">{String(contract.title)}</h2>
                      <p className="text-sm text-gray-500">ID: {String(contract.id).slice(0,8)} • {contract.paid ? 'PAID (Escrow)' : 'Payment Pending'}</p>
                 </div>
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
                ...
                <button onClick={onClose} className="w-full py-3 text-gray-500 hover:text-gray-800 text-sm font-medium">Cancel</button>
            </div>
        </div>
    );
}
...
function MessageSystem({ user, users, db, appId, channelId }) {
    ...
    return (
        <div className="h-[calc(100vh-140px)] flex flex-col bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700">
            <div className="p-4 border-b dark:border-gray-700 font-bold flex items-center gap-2"><MessageCircle className="w-5 h-5"/> Community Chat</div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col-reverse">
                {msgs.map(m => (
                    <div key={m.id} className={`flex gap-3 max-w-[80%] ${m.senderId === user.id ? 'self-end flex-row-reverse' : ''}`}>
                        <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center shrink-0 text-sm shadow-sm">{m.senderPic}</div>
                        <div className={`p-3 rounded-2xl ${m.senderId === user.id ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-gray-100 dark:bg-gray-700 rounded-tl-none'}`}>
                ...
                <div className="px-8 relative">
                    ...
                    <div className="mt-4 flex justify-between items-start">
                        <div>
                            <h1 className="text-3xl font-bold flex items-center gap-2">
                                {profile.name}
                                {/* Feature 4: Badge Visibility in Header */}
                                {profile.verified && <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Verified</span>}
                            </h1>
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-8 mb-8">
                        ...
                        <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-2xl text-center">
                            ...
                            <div className="text-xs text-gray-400 uppercase">Jobs Done</div>
                        </div>
                        <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-2xl text-center">
                            <div className="text-2xl font-bold text-green-500">{profile.verified ? '100%' : '50%'}</div>
                            <div className="text-xs text-gray-400 uppercase">Trust Score</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl w-full max-w-lg shadow-2xl animate-fade-in-up max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl mb-6">
                    <button onClick={() => setJobType('standard')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition ${jobType === 'standard' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`}>One-Time Fix</button>
                    <button onClick={() => setJobType('contract')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition ${jobType === 'contract' ? 'bg-white shadow text-purple-600' : 'text-gray-500'}`}>Long-Term Contract</button>
                </div>
                        ...
                        {!isLogin && (
                            <div className="flex p-1 bg-white/5 rounded-xl mb-6">
                                {['company', 'engineer'].map(t => (
                                    <button key={t} onClick={() => setType(t)} className={`flex-1 py-2 text-sm font-medium rounded-lg capitalize transition-all ${type === t ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>{t}</button>
                                ))}
                            </div>
                        )}
...
export function PremiumOfferCards({ onSelect, currentPlan }) {
  const plans = [
    { 
      ...
      duration: '3 Months', 
      price: 87, 
      originalPrice: 116, // Calculated based on monthly $29 * 4 (3 + 1 free logic)
      label: '1 Month Free', 
      color: 'blue' 
    },
    { 
      ...
      duration: '6 Months', 
      price: 174, 
      originalPrice: 232, // Monthly * 8 (6 + 2 free logic)
      label: '2 Months Free', 
      color: 'purple' 
    },
    { 
      ...
      duration: '1 Year', 
      price: 261, 
      originalPrice: 435, // Monthly * 15 (12 + 3 free logic)
    ...
    <div className="grid gap-3 md:grid-cols-3 mb-6">
      {plans.map(plan => (
        <div 
          ...
          className={`relative p-4 border-2 rounded-xl cursor-pointer transition hover:scale-105 ${currentPlan === plan.id ? `border-${plan.color}-500 bg-${plan.color}-50 dark:bg-${plan.color}-900/20` : 'border-gray-100 dark:border-gray-700'}`}
        >
          <div className={`absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm whitespace-nowrap bg-${plan.color}-100 text-${plan.color}-700`}>
            {plan.label}
          </div>
